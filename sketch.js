...
let showMenu = false; // Estado para alternar entre la pantalla principal y el menú (por defecto el menú está oculto)
...
function drawMainScreen() {
  ...
  drawLight(lightX, lightY, 50); // Escalar el tamaño del halo
...
function drawLight(x, y, maxSize) {
  for (let i = maxSize; i > 0; i -= 70) { // Ajustar degradado del halo
...
function drawInteractiveCircle(circle, lightX, lightY) {
  ...
  let brightness = map(d, 0, 400, maxBrightness, 0); // Ajustar rango de iluminación
  brightness = constrain(brightness, 0, maxBrightness);

  for (let i = circle.r * 2; i > 0; i -= 3) { // Ajustar densidad de los círculos
  ...
  if (circle.text && d < 200) { // Ajustar rango de detección para mostrar texto
...
function drawTransition() {
  ...
  if (progress < 0.5) {
    ...
    text("san.studio", width / 2, height / 2);
  } else {
    background(30);
    menuScale = lerp(0, 1, (progress - 0.3) * 2); // Coordinado con la transición de "portfolio"
    fill(255);
    textAlign(CENTER, CENTER);
    textSize(100 * 2); // El texto no crece más allá de este tamaño
    text("san.studio", width / 2, height / 2); // Mostrar texto estático
  }

  transitionProgress++;
  if (transitionProgress >= 200) {
    transitioning = false; // Finaliza la transición
    showMenu = true; // Muestra el menú después de la transición
  }
}
...
function resetToMainScreen() {
  ...
  showMenu = false; // Escondemos el menú en el comienzo
  portfolioScale = 0;
  menuScale = 0;
}
...
function mousePressed() {
  if (showMenu) {
    ...
    for (let option of menuOptions) {
      let y = height / 2 - 200 + menuOptions.indexOf(option) * 100;
      if (mouseY > y - 20 && mouseY < y + 20) {
        if (option === "about me") {
          // Iniciar la transición hacia la pantalla inicial
          resetToMainScreen();
        }
      }
    }
  ...
  // Si estamos en la pantalla principal y se hace clic en "portfolio"
